...
class ImageStandardizer:
    ...
    def fit(self, X: npt.NDArray) -> None:
        ...
        self.image_mean = np.array([X[:, :, :, c].mean() for c in range(X.shape[3])])
        self.image_std = np.array([X[:, :, :, c].std() for c in range(X.shape[3])])
    
    def transform(self, X: npt.NDArray) -> npt.NDArray:
        """Return standardized dataset given dataset X."""
        X = np.array(X, dtype=np.float64)
        for c in range(X.shape[3]):
            X[:, :, :, c] = (X[:, :, :, c] - self.image_mean[c]) / self.image_std[c]
        return X
...
class DogsDataset(Dataset):
    ...
    def __init__(self, partition: str, task: str = "target") -> None:
        ...
        self.transform = None
        if partition == "train":
            # Only apply augmentation on the training partition
            ...
            self.transform = T.Compose([
                # T.RandomResizedCrop(size=64, scale=(0.8, 1.0)),  # random crop and resize
                # T.RandomHorizontalFlip(),
                # T.RandomRotation(degrees=15),
                # T.ColorJitter(brightness=0.2, contrast=0.2),
                # T.ToTensor(),
                # T.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # normalize (using ImageNet mean/std as common values)
            ])
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # If this is the training partition, apply transformations
