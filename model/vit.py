...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Define the foward pass of the Transformer Encoder block as illustrated in Figure 4 of the spec.
        # Don't forget about the residual connections!
        norm = self.norm1(x)
        attention_output = self.multi_head_attention(norm)
...
class ViT(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        # Get linear projection of each patch to a token (hint: the necessary layers might already be defined!)
